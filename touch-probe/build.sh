#!/usr/bin/env bash
#
# Build (and optionally install) Touch Probe -- draws every active pointer and
# counts concurrent contacts, so "how many independent touches does Android
# actually receive here?" is measured rather than inferred.
#
# Why it exists: the SYNA7500 panel reports ABS_MT_SLOT max=9 and Waydroid's
# hwcomposer bridge reports Slot max=9 to match, but Android declares only
# android.hardware.touchscreen and android.hardware.faketouch -- none of the
# three touchscreen.multitouch* features. Before adding that feature XML to the
# overlay, the ten contacts have to be shown arriving as ten pointers.
#
# It doubles as the stylus probe: the HUD names TOOL_TYPE, the stylus button
# bits and hover distance per pointer, and names the device each event came from
# (wayland_touch / wayland_pointer / wayland_tablet), so a pen is distinguishable
# from another finger at a glance.
#
# Same no-Gradle toolchain under build/android as sensor-app/build.sh,
# quat-monitor/build.sh and drm-probe/build.sh, so --deps in any of them covers
# all of them:
#
#   aapt2 compile/link -> kotlinc -> d8 -> zipalign -> apksigner
#
# Usage:
#   touch-probe/build.sh              # build the APK
#   touch-probe/build.sh --deps       # fetch SDK + Kotlin compiler first
#   touch-probe/build.sh --install    # build, install and launch it
#   touch-probe/build.sh --pull       # copy the report off the device
#
# --install deliberately does NOT pull: the number only means something after
# someone has had their fingers on the glass. Touch first, then --pull.
#
# Env: HOST=<ip>  OUT=<dir>
set -euo pipefail
...
pull_report() {
	local dest="$repo/build/touch-probe-report"
	mkdir -p "$dest"
	ssh "$HOST" "sudo cat '$REPORTDIR/touch-probe.txt' 2>/dev/null" > "$dest/touch-probe.txt"
	if [ -s "$dest/touch-probe.txt" ]; then
		echo "== report at $dest/touch-probe.txt"
		echo
		cat "$dest/touch-probe.txt"
	else
		echo "== no report yet -- launch it and touch the screen first" >&2
	fi
}
...
if [ ! -f "$KEYSTORE" ]; then
	...
	keytool -genkeypair -keystore "$KEYSTORE" -alias debug \
		...
		-dname "CN=bigtab01 touch-probe debug, OU=none, O=none, C=CA" >/dev/null 2>&1
fi
...
if [ "$DO_INSTALL" = 1 ]; then
	...
	echo
	echo "== Touch Probe is up. Put as many fingers on the glass as you can;"
	echo "   the MAX counter in the HUD is the answer. Then:"
	echo "     touch-probe/build.sh --pull"
fi
